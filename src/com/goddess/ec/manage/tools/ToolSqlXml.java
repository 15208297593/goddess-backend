...
/**
 * 处理Sql Map<br/>
 * 加载sql map中的sql到map中，并提供动态长度sql处理
 */
...
//		}
	...
	public static String getSql(String sqlId, Map<String, String> param, LinkedList<Object> list) {
		...
		while (matcher.find()) {
			...
			} else if (clounm.equals("#(" + clounm2 + ")#")){ // IN查询
				String val = (String) param.get(clounm3);
				String[] paramArr = val.split("_");
				list.addAll(Arrays.asList(paramArr));
				
				sql = sql.replace(clounm, ToolSqlFormatter.formatInPart(paramArr.length));
	...
	public static void main(String[] args) {
		...
		Map<String, Object> paramMap = new HashMap<String, Object>();
		paramMap.put("xx", "yy");
		String result = BeetlKit.render("${xx}", paramMap);
	...
	public static void init() {
		File file = new File((ToolSqlXml.class.getClassLoader().getResource("").getFile()).replace("%20"," "));
